...
async def analyze(request: AnalyzeRequest, db: AsyncSession = Depends(get_db), payload: dict = Depends(verify_token)):
    ...
    article = AnalyzedArticle(user_id=payload["sub"], url=str(request.url))
    db.add(article)
    await db.commit()
    await db.refresh(article)

    word_weights = [WordWeight(article_id=article.id, word=w["word"], weight=w["weight"]) for w in records]
